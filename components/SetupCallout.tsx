export function SetupCallout({ kind }: { kind: "sanity" | "cloudinary" | "both" }) {
  const showSanity = kind === "sanity" || kind === "both";
  const showCloud = kind === "cloudinary" || kind === "both";
  return (
    <div className="mx-auto max-w-2xl rounded-lg border border-amber-200 bg-amber-50/90 p-6 text-amber-950 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-100">
      ...
      <ul className="mt-3 list-inside list-disc text-sm text-amber-900/90 dark:text-amber-200/90">
        ...
        {showCloud && (
          <li>
            <code className="text-xs">NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME</code> from your Cloudinary dashboard (images are
            served from the public id stored in Sanity).
          </li>
        )}
      </ul>
    </div>
